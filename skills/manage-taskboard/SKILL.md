...
When writing Chinese, keep the English word or use **本機 companion** / **本機配套服務** / **回送代理**. Never translate as **伴侶** or invent **伴侶 API**. Ordinary task/comment/attachment HTTP routes (`/api/tasks`, `/api/comments`, `/api/attachments`, …) are the **Taskboard HTTP API** (or local server API)—not “companion API”.
