...
async function resolveTaskboardBaseUrl(env, overrides) {
  ...
  const launcherRuntimeFile = defaultLauncherRuntimeFile(env);
  const readRuntimeFile = overrides.runtimeReadFile ?? readFile;
    ...
    : isWsl && wslRuntimeFile !== undefined
      ...
      : [
        ...
        ...([launcherRuntimeFile]
          ...
          .map((descriptorPath) => ({
            path: descriptorPath,
            // 使用者層級 launcher 端點走真實檔案系統，避免測試用 readFile mock
            // 攔截到 description、attachment 等無關檔案讀取。
            read: readRuntimeFile,
            required: false,
            windowsTransport: false,
          }))),
        {
