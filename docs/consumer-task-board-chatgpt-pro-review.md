...
- “其他任務”面板預設關閉，可開啟和關閉。
- 四個狀態 Tab、計數和內容正確。
- 面板任務可進入詳細資料，回到面板後會保留作用中的分頁。
- 搜尋同時作用於主列和側面板。
- 面板任務遷移到主列後，重新整理頁面仍保持新狀態。

未自動執行原生指標拖拽。目前瀏覽器驅動未暴露拖拽動作；已檢查真實拖拽鏈路仍復用 `TaskCard` 的 `dataTransfer` 和 `BoardColumn` 的原有 `onDrop`、排序及持久化流程。

## 目前狀態

- 修改只存在於本機 `feature/consumer-task-board` 分支工作樹。
- 未提交、未推送、未建立 PR、未部署。
- 未遷移資料庫，未修改線上設定，未操作真實使用者資料。
