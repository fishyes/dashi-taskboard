        ...
        <div className="task-form-dock">
          <div className="property-row">
            ...
            <div className="composer-menu-anchor" ref={moreMenuRef}>
              <button className="property-control property-more" type="button" aria-label={text("更多屬性", "More properties")} onClick={() => { setRelationMenu(null); setMenu(menu === "more" ? null : "more"); }}><LinearIcon name="more" /></button>
              {menu === "more" && (
                <div className="composer-popover more-popover" role="menu">
                  <button type="button" onClick={() => setMenu("due")}><span><LinearIcon name="calendarAdd" /></span><strong>{text("設定截止日期", "Set due date")}</strong><kbd>⇧ D</kbd><b><LinearIcon name="chevronRight" /></b></button>
                  <button type="button" onClick={() => setMenu("recurrence")}><span><LinearIcon name="recurrence" /></span><strong>{text("設定重複…", "Set recurrence…")}</strong><b><LinearIcon name="chevronRight" /></b></button>
                  {!task && (
                    <>
                      <div className="more-popover-divider" />
                      <button className={relationMenu === "subIssue" ? "is-open" : undefined} type="button" role="menuitem" aria-haspopup="menu" aria-expanded={relationMenu === "subIssue"} onClick={() => setRelationMenu("subIssue")}><span><LinearIcon name="plus" /></span><strong>{text("新增子議題", "Add sub-issue")}</strong>{selectedSubIssues.length > 0 && <small>{text(`${selectedSubIssues.length} 個已選`, `${selectedSubIssues.length} selected`)}</small>}<b><LinearIcon name="chevronRight" /></b></button>
              ...
              {menu === "due" && (
                <div className="composer-popover due-popover">
                  <label className="custom-date-row"><span>{text("自訂…", "Custom…")}</span><input type="date" value={dueDate} onChange={(event) => chooseDueDate(event.target.value)} /></label>
