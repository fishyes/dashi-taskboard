  ...
  return (
    ...
    >
      ...
      >
        ...
        <label>
          <span>{text("Jira 網址", "Jira URL")}</span>
          <input
            autoFocus
            required
            inputMode="url"
        ...
        {/^http:\/\//i.test(baseUrl.trim()) && (
          <p className="jira-http-warning">
            {text(
              "HTTP 會在內部網路中以可讀取的形式傳輸帳號密碼。",
              "HTTP sends the username and password in cleartext over the network.",
            )}
          </p>
        )}
        <label>
          <span>{text("Jira 專案（名稱或 Key，可複選）", "Jira projects (name or key, multiple allowed)")}</span>
          <input
            maxLength={2600}
        ...
        <label>
          ...
          <input
            ...
            maxLength={254}
            placeholder={connection?.configured ? text("留空則維持不變", "Leave blank to keep unchanged") : ""}
            value={username}
