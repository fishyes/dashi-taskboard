...
export function BoardCardDisplayMenu({
  ...
  const menu = open ? createPortal(
    <div
      ...
      role="dialog"
      aria-label={text("卡片顯示設定", "Card display settings")}
      style={{ left: position.left, top: position.top, visibility: position.ready ? "visible" : "hidden" }}
    >
      <div className="project-automation-menu-heading">
        <strong>{text("卡片顯示", "Card display")}</strong>
      </div>
      ...
      <div className="project-automation-switch">
        <span>{text("內文", "Body")}</span>
        <button
          type="button"
          className={`board-setting-switch${body ? " is-on" : ""}`}
          role="switch"
          aria-label={text("內文", "Body")}
          aria-checked={body}
