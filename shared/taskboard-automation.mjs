...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ? [
        ...
        "確認允許開始後，只有未綁定且仍為未歸檔 todo 的議題才可在讀取程式碼、下載附件、分析或實施前，由目前本機控制器使用剛讀取的 version 移到 in_progress。已有完整 threadBinding 時，issue move 必須同時傳入 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path 的儲存值，但在舊會話 send/stale 判斷完成前不得將這個 todo 移到 in_progress；stale 清除步驟按後文明確使用 --clear-binding-thread。未綁定時必須傳入 --clear-binding-thread，避免將本機控制器 CODEX_THREAD_ID 寫成任務綁定。寫入成功後記錄回應 task 的 version 為 ownedVersion、projectId 為 ownedProjectId，並記錄本輪 binding；此後本輪每次 issue move 都必須明確傳入 --if-version ownedVersion，成功後再用回應 version 更新 ownedVersion。不得省略 --if-version 後讓 taskctl 自動讀取最新 version。寫入成功前不得繼續。所有認領、評論和狀態寫入只由目前本機控制器完成，不得要求遠端會話執行 taskctl。",
        ...
        "認領成功後，已有完整 threadBinding 時，只能使用其儲存的 threadId 和 codexHostId 呼叫 Codex send_message_to_thread。send 成功後必須重新執行 issue get 一次，確認 projectId 未變、未歸檔、status 仍為 todo 且完整 threadBinding 與儲存值完全相同；然後由目前本機控制器使用這次複核回傳的最新 version、完整舊 binding 和 --if-version 執行 issue move --status in_progress，傳入 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path，並記錄回應 task.version 為 ownedVersion。認領成功後繼續執行後文既有 Codex wait_threads、結果評論和 in_review 寫回路徑，不得結束本輪；若認領發生 409，立即停止，不得重讀新 version 覆蓋。只有舊會話工具明確回傳終態 NOT_FOUND 或 CLOSED 等會話不存在或已關閉結果時，才確認 stale。timeout、network failure、Codex host 暫時無法連線或 Taskboard service unavailable 都不是 stale：保留 binding 並結束本輪，不得猜測、clear、create 或搶佔。若任務已是 in_progress、活躍、已歸檔、狀態或 binding 已變化，立即停止，不得在目前自動化目標建立替代會話。只有未綁定議題才使用 Codex create_thread 建立遠端任務，target 必須是 {type:\"project\",projectId:actualTarget.codexProjectId,environment:{type:\"local\"}}，首次 identity 必須使用 actualTarget 的 projectId、kind=\"remote\"、hostId 和 workspacePath。傳送給遠端會話的指令必須包含議題編號、標題、完整描述、全部評論和開發上下文，並說明遠端會話不執行 taskctl，只需完成實作、驗證並回傳改動、結果和剩餘風險。",
        "確認舊會話 stale 後，必須先用 comment add 儲存一筆歷史記錄，並同時傳入 --thread-id、--binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id 和 --binding-workspace-path 的完整舊 binding；評論寫入成功後，再使用同一次 issue get 的 version 執行 issue move --status todo --clear-binding-thread --if-version。評論或清除失敗立即停止，不得認領。然後只重新執行 issue get 一次；僅當 projectId 未變、未歸檔、status 仍為 todo、threadId 為空且 threadBinding 為空時，才進入未綁定議題的既有認領和 create_thread 路徑。",
    ...
    : [
        ...candidateInstructions,
        `確認允許開始後，只有 threadId 和 threadBinding 都為空且仍為未歸檔 todo 的議題，才可在讀取程式碼、下載附件、分析或實施前認領。認領必須使用剛讀取的 version 移到 in_progress，並明確傳入 --binding-thread-id "$CODEX_THREAD_ID"、--binding-codex-project-id ${JSON.stringify(request.codexProjectId)}、--binding-codex-project-kind "local"、--binding-codex-host-id ${JSON.stringify(request.codexHostId)}、--binding-workspace-path ${JSON.stringify(request.workspacePath)}，將目前自動化會話一次寫成完整 binding；記錄回應 task.version 為 ownedVersion。寫入成功前不得繼續。已有完整 binding 或 legacy local binding 的議題必須先按舊會話規則處理，不得先認領；不得認領已被其他會話綁定或其他 Agent 領取的議題。認領後的每一次 issue move 都必須明確傳入 ownedVersion 和這五個完整 binding 欄位，成功後更新 ownedVersion。`,
        ...
        `若首次 issue get 回傳完整 threadBinding，議題已綁定原會話：不要在目前自動化會話認領；只能使用儲存的 threadId 和 codexHostId 呼叫 Codex send_message_to_thread。send 成功時保留 binding 並結束本輪；只有工具明確回傳終態 NOT_FOUND 或 CLOSED 等會話不存在或已關閉結果時才確認 stale。timeout、network failure、Codex host 暫時無法連線或 Taskboard service unavailable 都保留 binding 並結束本輪，不得猜測 stale。確認 stale 後，先用 comment add 同時傳入 --thread-id 和完整舊 binding 儲存歷史，再用同一次 issue get 的 version 執行 issue move --status todo --clear-binding-thread --if-version；然後只重新執行 issue get 一次，仍為未歸檔 todo 且 threadId、threadBinding 都為空時，才在目前自動化會話處理。若任務已是 in_progress、活躍、已歸檔、狀態或 binding 已變化，或發生 409，立即停止，不得搶佔。若回傳 threadId 但沒有完整 threadBinding，這是 legacy local 綁定：先呼叫 Codex list_threads（limit=50），合併 pinnedThreads 與 threads，並按完整 threadId 精確尋找。只有恰好一項 kind="codex"、projectId=${JSON.stringify(request.codexProjectId)}、hostId=${JSON.stringify(request.codexHostId)}、cwd=${JSON.stringify(request.workspacePath)} 全部一致時，才將該項視為可核驗舊會話；使用最新 issue version 執行 issue move --status todo --if-version，並明確傳入舊 threadId 及上述 projectId、kind="local"、hostId、workspacePath 五欄位，將 legacy local 原地升級為完整 binding。升級成功後只向該舊 threadId 和 hostId 呼叫 send_message_to_thread，隨後結束本輪，由舊會話按議題最新要求繼續。若 list_threads 未找到、出現多項或任一欄位不一致，不得遷移或傳送；使用 comment add 記錄實際不一致項目，再用首次讀取的 version 和 --if-version、--binding-thread-id 保留原 threadId，將議題移到 blocked。若升級發生 409，立即停止，不得用新 version 覆蓋。若沒有 threadId，則按未綁定議題處理。`,
        "若議題已綁定 branch 或 worktree，必須在該議題綁定的開發上下文執行，避免並行 Agent 修改同一工作目錄。",
        "執行完成並驗證後，先用 comment add 記錄關鍵改動、驗證結果、執行結果和剩餘風險，再使用 ownedVersion、明確的 --if-version 和認領時儲存的完整 binding 將議題移動到 in_review；成功後更新 ownedVersion。不要省略 binding，避免將完整綁定降級為 legacy local；不要直接標記為 done。",
      ];
  return [
