...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ...
    : [
        ...
        "每次僅處理一個符合依賴條件的 todo：選定後先用 issue get 讀取最新議題內容，並用 comment list 讀取全部評論。根據描述和最新評論判斷是否允許開始；若其中寫明等待、暫不執行或目前不應開始，立即跳過並報告，不改狀態。評論也包含已完成後被打回的返工要求。",
        "完成 issue get 和 comment list 後、移動狀態前，必須再次執行 issue get，並複核 relations.blockedBy 仍為空或其中每個依賴的 status 都嚴格等於 done。若依賴條件不再符合，立即跳過並結束本輪，不改狀態，也不暫停自動化。",
        "確認允許開始後，只有未綁定且仍為未歸檔 todo 的議題才可在讀取程式碼、下載附件、分析或實施前，使用剛讀取的 version 移到 in_progress；寫入成功前不得繼續。已有 binding 的議題必須先按舊會話 send/stale 規則處理，不得先認領；不得認領已被其他會話綁定或其他 Agent 領取的議題。",
        ...
        "若首次 issue get 回傳完整 threadBinding，議題已綁定原會話：不要在目前自動化會話認領；只能使用儲存的 threadId 和 codexHostId 呼叫 Codex send_message_to_thread。send 成功時保留 binding 並結束本輪；只有工具明確回傳終態 NOT_FOUND 或 CLOSED 等會話不存在或已關閉結果時才確認 stale。timeout、network failure、Codex host 暫時無法連線或 Taskboard service unavailable 都保留 binding 並結束本輪，不得猜測 stale。確認 stale 後，先用 comment add 同時傳入 --thread-id 和完整舊 binding 儲存歷史，再用同一次 issue get 的 version 執行 issue move --status todo --clear-binding-thread --if-version；然後只重新執行 issue get 一次，仍為未歸檔 todo 且 threadId、threadBinding 都為空時，才在目前自動化會話處理。若任務已是 in_progress、活躍、已歸檔、狀態或 binding 已變化，或發生 409，立即停止，不得搶佔。若回傳 threadId 但沒有完整 threadBinding，這是舊版本機綁定：無法確認專案和主機，不得 send、create 或覆蓋該綁定；使用 comment add 記錄原因，再用首次讀取的 version 和 --if-version、--binding-thread-id 保留原 threadId，將議題移到 blocked。若沒有 threadId，則在目前自動化會話處理。",
