...
export function normalizeJiraUrl(value) {
  if (typeof value !== "string" || value.includes("?") || value.includes("#")) {
    throw new JiraConfigError(
      "INVALID_JIRA_URL",
      "Jira 網址必須使用 http 或 https，且不得包含帳號、查詢參數或片段",
    );
  }
  let url;
  try {
    ...
    throw new JiraConfigError("INVALID_JIRA_URL", "Jira 網址無效");
  }
  if (
  ...
  const projects = value.map((project) => {
    if (
      ...
      throw new JiraConfigError(
        ...
        "Jira 專案名稱或 Key 不得為空、包含控制字元，且不得超過 128 個字元",
      );
    }
    return project.trim();
  });
...
function parseConfig(value) {
  ...
  if (
    ...
    throw new JiraConfigError("INVALID_JIRA_CONFIG", "Jira 設定缺少穩定的執行個體身分");
  }
