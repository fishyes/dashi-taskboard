    ...
    try {
      ...
      if (projectLabelsRoute) {
        ...
        if (request.method === "DELETE" && projectId === JIRA_PROJECT_ID) {
          throw new ApiError(
            ...
            "Jira 標籤目錄由同步管理，無法在 Taskboard 中刪除",
          );
        }
        ...
        if (request.method === "POST") {
          ...
          if (input.projectId === JIRA_PROJECT_ID) {
            throw new ApiError(
              ...
              "請在 Jira 中新增議題，Taskboard 目前只同步已指派給你的任務",
            );
          }
      ...
      if (taskRoute) {
        ...
        if (!action && request.method === "PATCH") {
          ...
          if (current.source === "jira") {
            ...
            if (Object.hasOwn(changes, "projectId")) {
              throw new ApiError(409, "JIRA_PROJECT_MOVE_UNAVAILABLE", "Jira 任務無法移到本機專案");
            }
        ...
        if (!action && request.method === "DELETE") {
          const current = database.getTask(id);
          if (current?.source === "jira") {
            throw new ApiError(409, "JIRA_DELETE_UNAVAILABLE", "Jira 任務無法從 Taskboard 永久刪除");
          }
          const { version } = parseArchive(await readJson(request));
        ...
        if (action === "archive" && request.method === "POST") {
          const current = database.getTask(id);
          if (current?.source === "jira") {
            throw new ApiError(409, "JIRA_ARCHIVE_UNAVAILABLE", "Jira 任務由同步範圍自動管理，無法手動歸檔");
          }
          const { version, threadId, threadBinding } = resolveInputThreadBinding(
            parseArchive(await readJson(request)),
          );
        ...
        if (action === "restore" && request.method === "POST") {
          const current = database.getTask(id);
          if (current?.source === "jira") {
            throw new ApiError(409, "JIRA_RESTORE_UNAVAILABLE", "Jira 任務由同步範圍自動管理，無法手動恢復");
          }
          const { version, threadId, threadBinding } = resolveInputThreadBinding(
            parseArchive(await readJson(request)),
          );
