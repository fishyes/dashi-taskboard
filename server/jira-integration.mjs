...
function jiraOriginId(manifest) {
  ...
  if (!applicationId) {
    throw new ApiError(502, "INVALID_JIRA_ORIGIN", "Jira 未傳回穩定的執行個體身分");
  }
  ...
  async function request(config, pathname, init = {}) {
    ...
    try {
      ...
      throw new ApiError(
        ...
        timedOut ? "連線 Jira 逾時" : "無法連線 Jira，請檢查網址與內部網路連線",
      );
    } finally {
    ...
    if (response.status >= 300 && response.status < 400) {
      throw new ApiError(400, "JIRA_REDIRECT", "Jira 網址發生重新導向，請填寫最終存取網址");
    }
    if (!response.ok) {
      throw new ApiError(
    ...
    try {
      ...
      throw new ApiError(502, "INVALID_JIRA_RESPONSE", "Jira 傳回無效的 JSON 資料");
    }
  }
  ...
  async function assertLiveOrigin(config) {
    if (await fetchOriginId(config) !== config.originId) {
      throw new ApiError(
        ...
        "目前的 Jira 網址指向另一個執行個體，請重新連線後再操作",
      );
    }
  }
  ...
  async function resolveTransition(config, issueKey, targetStatus) {
    ...
    if (matches.length === 0) {
      throw new ApiError(
        ...
        `目前的 Jira 工作流程無法將 ${issueKey} 移到目標狀態`,
        { availableStatuses },
      );
    }
    if (matches.length > 1) {
